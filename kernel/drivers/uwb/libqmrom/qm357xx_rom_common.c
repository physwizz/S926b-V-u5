			      ...
			      struct unstitched_firmware *all_fws)
{
	...
	all_fws->key2_crt->data = all_fws->key1_crt->data + CRYPTO_IMAGES_CERT_KEY_SIZE;
	all_fws->key2_crt->size = CRYPTO_IMAGES_CERT_KEY_SIZE;

	all_fws->fw_crt->data = all_fws->key2_crt->data + CRYPTO_IMAGES_CERT_KEY_SIZE;
	all_fws->fw_crt->size = CRYPTO_IMAGES_CERT_CONTENT_SIZE;

	all_fws->fw_img->data =
		(const uint8_t *)fw_pkg_img_hdr + fw_pkg_img_hdr->descs[0].offset;
