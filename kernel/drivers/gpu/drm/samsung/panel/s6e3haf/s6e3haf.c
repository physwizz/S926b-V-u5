...
__visible_for_testing int get_mafpc_scale_index(struct mafpc_device *mafpc, struct panel_device *panel)
{
	...
	br_index = get_subdev_actual_brightness_index(panel_bl,
			PANEL_BL_SUBDEV_TYPE_DISP,
			panel_bl->subdev[PANEL_BL_SUBDEV_TYPE_DISP].brightness);
	if (br_index < 0) {
		panel_err("mafpc invalid scale index : %d\n", br_index);
		goto err_get_scale;
	}
...
int s6e3haf_get_temperature_range(struct panel_device *panel, void *buf)
{
	...
	snprintf(buf, PAGE_SIZE, "-15, -14, -13, -12, -11, -10, -9, -8, -7, -6, -5, -4, 0, 1, 29, 30\n");
	return 0;
}
